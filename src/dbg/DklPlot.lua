...
require "dbg/DklTriangleBarChart"
require "dbg/DklRadialColunmChart"
...
-- The following code adds functionality

function DklBaseGraphics:radial(labels,data,args)
	args = args or {}
	local ylim = ylim or range(data)
	self:plot_new()
	self:plot_window({0,#labels},ylim,args)
	...
	translate(self.fig.xoff,self.fig.yoff)
	translate(self.plt.xoff,self.plt.yoff)
	translate(self.fig.pin[1]*self.dev.res / 2, -self.fig.pin[2]*self.dev.res / 2)

	local radialDiameter = math.min(self.fig.pin[1]*self.dev.res, self.fig.pin[2]*self.dev.res)
	radialDiameter = radialDiameter * 5 / 4
	local increment = map(7, 0, radialDiameter / 2, ylim[1], ylim[2])

	local radialGraph = RadialColunmChart()
	radialGraph.setMinValue(ylim[1])
	radialGraph.setMaxValue(ylim[2])
	radialGraph.setIncrement(increment)
	radialGraph.setDiameter(radialDiameter)
	radialGraph.setLabels(labels)
	radialGraph.setData(data)
	radialGraph.setEvent(PRESSED)
	if args.col ~= nil then
		radialGraph.setColors(args.col)
	end
	radialGraph.setColumnWidth(radialDiameter * 0.05)
	radialGraph.draw()
	popMatrix()
...
function DklBaseGraphics:triangle(labels,data,args)
	args = args or {}
	local ylim = ylim or range(data)
	self:plot_new()
	self:plot_window({0,#labels},ylim,args)
	...
	translate(self.fig.xoff,self.fig.yoff)
	translate(self.plt.xoff,self.plt.yoff)
	translate(-self.plt.usr[1]*self.plt.xscl,self.plt.usr[3]*self.plt.yscl - 10)

	if ylim[1] > 0 then ylim[1] = 0 end

	local increment = map(7, 0, self.plt.usr[3]*self.plt.yscl + 10, ylim[1], ylim[2])

	local triangleGraph = TriangleBarChart()
	triangleGraph.setMinValue(ylim[1])
	triangleGraph.setMaxValue(ylim[2])
	triangleGraph.setIncrement(increment)
	triangleGraph.setWidth(self.fig.pin[1]*self.dev.res)
	triangleGraph.setHeight(self.fig.pin[2]*self.dev.res)
	triangleGraph.setOrientation(VERTICAL)
	triangleGraph.setLabels(labels)
	triangleGraph.setData(data)
	triangleGraph.setEvent(PRESSED)
	if args.col ~= nil then
		triangleGraph.setColors(args.col)
	end
	triangleGraph.draw()
	popMatrix()
